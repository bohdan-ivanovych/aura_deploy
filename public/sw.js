...
self.addEventListener('install', (event) => {
  ...
  // Do NOT call self.skipWaiting() here — we use a user-triggered toast instead
  // to avoid silent reloads (the source of the "quiet blinking" bug).
});
...
/* ── Message: SKIP_WAITING — triggered by SWUpdateToast component ─── */
/* WHY: Instead of auto-updating (which causes the silent page blink), we wait
   until the user explicitly clicks "Refresh" in the update toast. Only then
   do we tell the waiting SW to take over. */
